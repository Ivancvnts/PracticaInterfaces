...
#include "UI/PowerUpsTree/PowerUpsTree.h"
...
void AUTAD_UI_FPSCharacter::BeginPlay()
{
	...
	if (PlayerController)
	{
		...
		PlayerController->bCinematicMode = true;
	}
	...
	if (PlayerHUDWidget)
	{
		...
		PowerUpsTreeInstance = CreateWidget<UPowerUpsTree>(GetWorld(), PowerUpsTreeWidget, FName("PowerUpsTree"));
		PowerUpsTreeInstance->AddToViewport();
		PowerUpsTreeInstance->SetVisibility(ESlateVisibility::Hidden);
		Cast<UPlayerHUD>(PlayerHUDInstance)->StartSplashAnimation();
	}
	else
	{
...
void AUTAD_UI_FPSCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		//Open & Close PowerUps Tree
		EnhancedInputComponent->BindAction(PowerUpsAction, ETriggerEvent::Triggered, this, &AUTAD_UI_FPSCharacter::OpenPowerUpsTree);
	}
}
...
void AUTAD_UI_FPSCharacter::OpenPowerUpsTree(const FInputActionValue& Value)
{
	if(!PowerUpsTreeInstance->isOpen)
	{
		PowerUpsTreeInstance->SetVisibility(ESlateVisibility::Visible);
		APlayerController* PlayerController = Cast<APlayerController>(Controller);
		PlayerController->SetPause(true);
		PlayerController->bShowMouseCursor = true;
		UWidgetBlueprintLibrary::SetInputMode_UIOnlyEx(PlayerController);
		PowerUpsTreeInstance->isOpen = true;
	}
