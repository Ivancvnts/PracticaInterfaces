...
void UTP_WeaponComponent::CompleteReload()
{
	...
	OnCurrentAmmoChange.Broadcast(CurrentNumBullets);
	OnTotalAmmoChange.Broadcast(playerBullets - CurrentNumBullets);
	ReloadTimer = 0.f;
	OnReload.Broadcast(ReloadTimer);
}
...
void UTP_WeaponComponent::AttachWeapon(AUTAD_UI_FPSCharacter* TargetCharacter)
{
	...
	//When Player gets the weapon, broadcast the initial ammo values to anyone who subscribes to delegate
	OnCurrentAmmoChange.Broadcast(CurrentNumBullets);
